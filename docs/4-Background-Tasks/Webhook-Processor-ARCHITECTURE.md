...
1. **Resolve the Verified State:** It obtains the authoritative state (e.g., `FULFILL`) — from the webhook payload when its signature verifies, otherwise by reading it from the Wallee API. See [Webhook Processor](Webhook-Processor.md#the-key-idea-state-driven-not-event-driven).
...
The states above are a `Transaction`'s, but the loop is entity-agnostic: a `Refund`, `DeliveryIndication`, `ManualTask` or any other webhook entity runs through the same path calculation against its own state machine.
...
> **Implementation Example:** See [src/MyExampleLifecycleHandler.php](../examples/4-Background-Tasks/webhook-processor/src/MyExampleLifecycleHandler.php) for a reference implementation of `getLockableResources` used to define these locks.

---

## 3. Persistence & Atomicity

Each step of the catch-up loop runs as `preProcess()` → *Command* → `postProcess()`, with the processor calling those hooks on your lifecycle handler:

1. **Pre-Process:** Acquires locks and re-checks the state (to skip duplicates). Return `false` to skip the step.
2. **Execute:** Runs the business logic Command.
3. **Post-Process:** Records the new state in your `webhook_progress` table and releases locks.

> **Terminology:** "transaction" below means a **database** transaction in your shop, not a Wallee `Transaction` (the payment entity). They are unrelated, and a webhook for any entity type — `Refund`, `DeliveryIndication`, `ManualTask` — needs the same database-level atomicity.

The processor does not open a database transaction itself — `DefaultWebhookLifecycleHandler` only locks and unlocks. Making a step atomic is your handler's job: override `preProcess()`/`postProcess()` to begin and commit a database transaction around it (always calling `parent`, so the locking still runs).

**Critical Rule:** The `webhook_progress` table is the source of truth for `plugin-core`. It must be updated atomically with the business logic — if the command succeeds but recording the state does not, the step is replayed on the next delivery.

---

## 4. Failure Handling & Retries

Any exception thrown during a step triggers the same recovery path: the processor calls `onFailure()` — which releases the locks, and rolls back a database transaction if your handler opened one — and re-throws a `CommandException`. The entry-point controller should translate this into a **5xx response**, which instructs the Wallee Portal to redeliver the webhook later.

### Transient Failures (Expected Contention)

Not every failure is a defect. Under concurrent deliveries, a lock acquisition can legitimately time out — the Wallee Portal retry will succeed on the next attempt. Throwing a generic exception in that situation floods the logs with `error` entries for a self-healing state.
...
* **Implementation Example**: See [src/Transaction/TransactionListener.php](../examples/4-Background-Tasks/webhook-processor/src/Transaction/TransactionListener.php) for a reference implementation of this safety check.
...
* [ ] **Lifecycle Handler:** Create a base handler extending `DefaultWebhookLifecycleHandler`.
  ...
  * [ ] Implement `preProcess` (start a database transaction) / `postProcess` (commit it & update the table).
