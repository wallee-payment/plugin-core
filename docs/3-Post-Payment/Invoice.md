...
* **`InvoiceService`**: The entry point consumers use — `find()`, `get()` and `search()`.
* **`InvoiceGatewayInterface`**: The read-only infrastructure port the service delegates to. Consumers do not call it directly.
...
See [fetch_invoice.php](../examples/3-Post-Payment/fetch_invoice.php) for a working CLI script that locates the invoice of a transaction, prints its state and amounts, and iterates the invoiced line items.

## Used by Refunds

Invoices are the source of truth behind `RefundService::getRefundableLineItems()` — it resolves the invoiced line items (via the latest successful refund's reduced state, or this invoice's line items when no refund exists yet) so callers never have to reason about invoices directly. See the **[Refund documentation](Refund.md)** for the full partial-refund calculation, including why `LineItem::$unitPriceIncludingTax` must be used instead of dividing.
