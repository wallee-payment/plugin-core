...
Neither method is eligibility-aware; for what to actually offer a customer during checkout, see [`TransactionService::getAvailablePaymentMethods()`](../Checkout/README.md#available-payment-methods).

`synchronize()` fetches the current methods from the portal, diffs them against what your plugin already has stored, and calls the matching operation on your repository — you never write the diffing logic yourself. To use it, implement `PaymentMethodRepositoryInterface`:
...
// ... setup dependencies, including your PaymentMethodRepositoryInterface implementation ...
...
See [example/2_synchronize_methods.php](example/2_synchronize_methods.php) for a full runnable script, including a sample repository implementation.
...
- [example/retrieval.php](example/retrieval.php): Fetches and displays the available payment methods.
- [example/2_synchronize_methods.php](example/2_synchronize_methods.php): Retrieves methods and synchronizes them into a sample repository (see [Synchronizing Payment Methods](#synchronizing-payment-methods) above).
