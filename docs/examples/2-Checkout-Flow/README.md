...
The demo consists of four sequential steps. Run them in order:
...
The simplest integration mode: redirects the user directly to the Wallee payment page.
```bash
php 5_confirm_payment_page.php
```
...
### Step 4: Inspect the Context Used
Reads the transaction back and prints the environment (space view, language) and payment method (configuration, connector, icon) it was actually processed with — the immutable snapshots an order page or receipt should display. Run it after paying via one of the Step 3 scripts, otherwise no payment method has been selected yet.
