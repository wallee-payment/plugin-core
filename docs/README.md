...
Without these, any example script exits immediately with a "Missing environment variable" error. Set them once per terminal session (or globally), then run any example from its own directory.
